...
switch ($act) {
	...
	case 'edit':
		...
		include(pe_tpl('order_add.html','admin'));
	break;
	//####################// 订单删除 //####################//
	case 'del':
		pe_token_match();
	...
	case 'pay':
		...
		include(pe_tpl('order_pay.html','admin'));
	break;
	//####################// 订单发货 //####################//
	case 'send':
		$order_id = pe_dbhold($_g_id);
		...
		include(pe_tpl('order_send.html','admin'));
	break;
	//####################// 订单确认收货 //####################//
	case 'success':
		pe_token_match();
	...
	case 'close':
		...
		include(pe_tpl('order_close.html','admin'));
	break;
	//####################// 订单改价格 //####################//
	case 'money':		
		$order_id = pe_dbhold($_g_id);
		...
		include(pe_tpl('order_money.html','admin'));
	break;
	//####################// 订单改地址 //####################//
	case 'address':
		$order_id = pe_dbhold($_g_id);
		...
		$info = $db->pe_select('order', array('order_id'=>$order_id));
		include(pe_tpl('order_address.html','admin'));
	break;
	//####################// 打印购物单 //####################//
	case 'print_product':
		...
		include(pe_tpl('order_print_product.html','admin'));
	break;
	//####################// 打印快递单 //####################//
	case 'print_express':
		$order_id = pe_dbhold($_g_id);
		...
		include(pe_tpl('order_print_express.html','admin'));
	break;
	//####################// 订单列表 //####################//
	default:
		...
		if ($act == 'excel_out') {
			...
			include(pe_tpl('order_list.html','admin'));
		}
	break;
}
?>