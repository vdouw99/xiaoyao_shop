...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			...
			if ($product_id = $db->pe_insert('product', pe_dbhold($_p_info, array('product_text')))) {
				...
				pe_success('添加成功!', 'webadmin.php?mod=product');
			}
			else {
				pe_error('添加失败...');
			}
		...
		include(pe_tpl('product_add.html','admin'));
	break;
	//####################// 商品修改 //####################//
	case 'edit':
	...
	case 'move':
		...
		include(pe_tpl('product_move.html','admin'));
	break;
	//####################// 选择规格 //####################//
	case 'rule':
		$ruledata_id = $_g_id ? explode(',', $_g_id) : array();
		$seo = pe_seo($menutitle='选择规格', '', '', 'admin');
		include(pe_tpl('product_rule.html','admin'));
	break;
	//####################// 生成规格 //####################//
	case 'datalist':
	case 'datalist_new':
	...
	case 'ask':
		...
		include(pe_tpl('product_ask.html','admin'));
	break;
	//####################// 快速评价 //####################//
	case 'comment':
		pe_lead('hook/product.hook.php');
		...
		include(pe_tpl('product_comment.html','admin'));
	break;
	//####################// 设置销量 //####################//
	case 'sell':
		$product_id = intval($_g_id);
		...
		include(pe_tpl('product_sell.html','admin'));
	break;
	//####################// 商品列表 //####################//
	default :
		...
		include(pe_tpl('product_list.html','admin'));
	break;
}
