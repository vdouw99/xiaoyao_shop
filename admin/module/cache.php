...
switch ($act) {
	...
	default:
		...
		$data_list = pe_dirlist("{$pe['path_root']}cache/*.cache.php");
		foreach ($data_list as $v) {
			$data_size += filesize("{$pe['path_root']}cache/{$v}");
		}
		...
		$info_list['template']['cache_size'] = round(pe_dirsize("{$pe['path_root']}cache/template")/1024, 1);
		//附件缓存
		$info_list['attachment']['cache_name'] = '附件缓存';
		$info_list['attachment']['cache_text'] = '附件缓存占用过多空间时，可更新此项。';
		$info_list['attachment']['cache_size'] = round(pe_dirsize("{$pe['path_root']}cache/attachment")/1024, 1);
		//图片缓存
		$info_list['thumb']['cache_name'] = '图片缓存';
		$info_list['thumb']['cache_text'] = '图片缓存占用过多空间时，可更新此项。';
		$info_list['thumb']['cache_size'] = round(pe_dirsize("{$pe['path_root']}cache/thumb")/1024, 1);

		$seo = pe_seo($menutitle='缓存管理', '', '', 'admin');
		include(pe_tpl('cache_list.html','admin'));
	break;
}
?>