...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_insert('userlevel', pe_dbhold($_p_info))) {
				...
				userlevel_callback();
				pe_success('添加成功!', 'webadmin.php?mod=userlevel');
			}
			else {
				pe_error('添加失败...');
			}
		...
		include(pe_tpl('userlevel_add.html','admin'));
	break;
	//####################// 等级修改 //####################//
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_update('userlevel', array('userlevel_id'=>$userlevel_id), pe_dbhold($_p_info))) {
				cache_write('userlevel');
				userlevel_callback();
				pe_success('修改成功!', 'webadmin.php?mod=userlevel');
			}
			else {
				pe_error('修改失败...');
			}
	...
	default :
		...
		include(pe_tpl('userlevel_list.html','admin'));
	break;
}
?>