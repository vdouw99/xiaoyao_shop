...
switch ($act) {
	...
	case 'add':
		if (isset($_p_pesubmit)) {
			pe_token_match();
			if ($_FILES['category_icon']['size']) {
				pe_lead('public/class/upload.class.php');
				$upload = new upload($_FILES['category_icon']);
				$_p_info['category_icon'] = $upload->filehost;
			}
			if ($category_id = $db->pe_insert('category', pe_dbhold($_p_info))) {
				cache_write('category');
				pe_success('添加成功!', 'webadmin.php?mod=category');
			}
			else {
				pe_error('添加失败!');
			}
		...
		include(pe_tpl('category_add.html','admin'));
	break;
	//####################// 修改分类 //####################//
	case 'edit':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_update('category', array('category_id'=>$category_id), pe_dbhold($_p_info))) {
				cache_write('category');
				pe_success('修改成功!', 'webadmin.php?mod=category');
			}
			else {
				pe_error('修改失败...');
			}
	...
	default :
		...
		include(pe_tpl('category_list.html','admin'));
	break;
}
?>