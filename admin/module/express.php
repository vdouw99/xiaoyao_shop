...
switch ($act) {
	...
	case 'add':
		...
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_insert('express', pe_dbhold($_p_info, array('express_tag')))) {
				//cache_write('express');
				pe_success('添加成功!', 'webadmin.php?mod=express');
			}
			else {
				pe_error('添加失败...');
			}
		...
		include(pe_tpl('express_add.html','admin'));
	break;
	//####################// 模板修改 //####################//
	case 'edit':
		$express_id = intval($_g_id);
		if (isset($_p_pesubmit)) {
			...
			if ($db->pe_update('express', array('express_id'=>$express_id), pe_dbhold($_p_info, array('express_tag')))) {
				//cache_write('express');
				pe_success('修改成功!', 'webadmin.php?mod=express');
			}
			else {
				pe_error('修改失败...');
			}
	...
	default :
		...
		include(pe_tpl('express_list.html','admin'));
	break;
}
?>