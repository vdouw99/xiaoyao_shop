...
switch ($act) {
	...
	case 'order':
		...
		include(pe_tpl('tongji_order.html','admin'));
	break;
	//####################// 热销排行 //####################//
	case 'sell':
		$time1 = $_g_date1 ? strtotime($_g_date1) : strtotime('-1 month');
		...
		include(pe_tpl('tongji_sell.html','admin'));
	break;
	//####################// 消费排行 //####################//
	case 'user':
		$time1 = $_g_date1 ? strtotime($_g_date1) : strtotime('-1 month');
		...
		include(pe_tpl('tongji_user.html','admin'));
	break;
	//####################// 短信/邮件记录 //####################//
	case 'notice':
	case 'notice_del':
