...
switch ($act) {
	...
	case 'point':
		...
		include(pe_tpl('setting_point.html','admin'));		
	break;
	//####################// 短信设置 //####################//
	case 'sms':
		if (isset($_p_pesubmit)) {
			...
			$sql = "update `".dbpre."setting` set `setting_value` = case `setting_key`
				...
				when 'sms_secret' then '".pe_dbhold($_p_info['sms_secret'])."'
		...
		include(pe_tpl('setting_sms.html','admin'));
	break;
	//####################// 短信测试 //####################//
	case 'sms_test':
		pe_token_match();
		...
		foreach (explode(',', $_g_user) as $k=>$v) {
			...
			$sql_set[$k]['noticelog_text'] = "【逍遥商城】尊敬的用户：您好，欢迎使用逍遥商城系统";
	...
	case 'email':
		...
		include(pe_tpl('setting_email.html','admin'));		
	break;
	//####################// 邮件测试 //####################//
	case 'email_test':
		pe_token_match();
		...
		foreach (explode(',', $_g_user) as $k=>$v) {
			...
			$sql_set[$k]['noticelog_user'] = pe_dbhold($v);
			$sql_set[$k]['noticelog_name'] = '逍遥商城系统测试邮件';
			$sql_set[$k]['noticelog_text'] = '尊敬的用户：您好，欢迎使用逍遥商城<br/><br/><br/>邮件发送日期：'.pe_date(time());
			$sql_set[$k]['noticelog_atime'] = time();			
		}
		if ($db->pe_insert('noticelog', $sql_set)) {
			pe_success('发送成功!');
	...
	case 'user':
		...
		include(pe_tpl('setting_user.html','admin'));
	break;
	//####################// 网站设置 //####################//
	default:
		if (isset($_p_pesubmit)) {
		...
		include(pe_tpl('setting_base.html','admin'));
	break;
}
?>