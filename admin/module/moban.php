...
switch ($act) {
	...
	case 'down':
		...
		$moban_name = file_get_contents("http://www.qiye1000.com/index.php?mod=api&act=moban_down&id={$moban_id}&type=mark");
		$moban_file = file_get_contents("http://www.qiye1000.com/index.php?mod=api&act=moban_down&id={$moban_id}");
		$moban_zip = "{$pe['path_root']}cache/thumb/{$moban_name}";	
		...
		$moban_zip_arr = explode("{$pe['path_root']}cache/thumb/", $moban_zip);
		...
		pe_lead('public/class/pclzip.class.php');
	...
	default:
		...
		include(pe_tpl('moban_list.html','admin'));
	break;
}
